...
function getAllDataFromDBWithRealtimeSync(
    database,
    documentID,
    path,
    classRef
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            collectionRef.onSnapshot((querySnapShot) => {
                //iterate over each query that was made on firestore collection underwatch
                ...
                for (let change of changes) {
                    if (change.type === 'added') {
                        //if document was added this is called. Also fetches all documents in first call
                    ...
                    if (change.type === 'modified') {
                        //if any document is modified
                        logger.debug('data modified');
                        classRef.addToMap(
                            documentID,
                            change.doc.id,
                            change.doc.data()
                        ); //replace or add use same function utlising replacement property of map
                    }
                    if (change.type === 'removed') {
                        //if any document is deleted
